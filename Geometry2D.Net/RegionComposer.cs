...
namespace Svd.Geometry2D
{
    internal class RegionComposer : IRegionComposer
    {
        public RegionComposer(IGeometry2DEngine geometry2DEngine)
        {
            ...
            vertexNodeMap = new SortedDictionary<Point2DD, VertexNode>(Point2DDComparerYX.Default);
            SetBoundingRect(AARect2DD.EmptyAABB);
        }

        private readonly IGeometry2DEngine geometry2DEngine;
        private readonly List<IPath> paths = new List<IPath>();
        private readonly SortedDictionary<Point2DD, VertexNode> vertexNodeMap;
        private readonly Dictionary<IPathSegment, SegmentNode> segmentNodeMap = new Dictionary<IPathSegment, SegmentNode>();
        private List<AreaInfo> areas;
        private AARect2DD boundingRect;
        private Point2DD areaSize;
        private Point2DD areasOrigin;
        ...
        private void Add(IPath path)
        {
            ...
            // add vertices
            var pathVertexIterator = new PathVertexIterator(path);
        ...
        public IRegion Compose(RegionCompositionMode mode)
        {
            ...
            ClearAllFlags();
            var outlines = new LinkedList<IPath>();
            var voids = new LinkedList<IPath>();
            foreach (var startVertexNode in vertexNodeMap.Values)
            {
                foreach (var startVertex in startVertexNode.Vertices)
                {
                    ...
                    var newPathPoints = new HashSet<Point2DD>();
            ...
            var resultOutlines = (IEnumerable<IPath>)outlines.Where(o => !voids.Any(v => geometry2DEngine.AreCoincident(o, v))).ToList();
            var resultVoids = (IEnumerable<IPath>)voids;
        ...
        private NextVertexInfo SelectVertex(VertexNode vertexNode, IPathSegment prevSegment, int prevPathWinding, RegionCompositionMode mode)
        {
        ...
        private void SplitIntersections(IPathSegment newSegment)
        {
        ...
        private bool SplitSegment(IPathSegment segment, Point2DD splitPoint)
        {
        ...
        private void SetBoundingRect(AARect2DD newBoundingRect)
        {
                ...
                double.IsInfinity(newBoundingRect.RT.Y))
            {
                ...
                areaSize = Point2DD.Zero;
                areasOrigin = Point2DD.Zero;
                areasCountX = 0;
                areasCountY = 0;
                return;
            }
            ...
            var newAreaSize = new Point2DD(newAreaWidth, newAreaHeight);
            ...
            for (int newAreaYIndex = 0; newAreaYIndex < newAreasCountY; newAreaYIndex++)
            {
                ...
                {
                    var newAreaRect = new AARect2DD(
                        newAreasOrigin + new Vector2DD(newAreaXIndex * newAreaWidth, newAreaYIndex * newAreaHeight),
                        newAreasOrigin + new Vector2DD((newAreaXIndex + 1) * newAreaWidth, (newAreaYIndex + 1) * newAreaHeight));
        ...
        private IEnumerable<IPathSegment> FindOriginalSegmentsAtRect(AARect2DD rect)
        {
            var processedOriginalSegments = new HashSet<IPathSegment>();
        ...
        private IEnumerable<IPath> FindOriginalPathsAtRect(AARect2DD rect)
        {
            var processedOriginalPaths = new HashSet<IPath>();
            foreach (var areaInfo in GetAreaInfosAtRect(rect))
            {
        ...
        private IEnumerable<AreaInfo> GetAreaInfosAtRect(AARect2DD rect)
        {
        ...
        private AARect2DI GetAreaIndicesAtRect(AARect2DD rect)
        {
            if (areasCountX == 0 || areasCountY == 0)
                return new AARect2DI() { LB = new Point2DI(0, 0), RT = new Point2DI(-1, -1) };
            ...
            return new AARect2DI(minX, minY, maxX, maxY);
        }
        ...
        private class PathVertexEx
        {
            ...
            public PathVertexEx(IPath path, PathVertex pathVertex)
            {
                Path = path;
            ...
            public IPathSegment PrevSegment { get; set; }
            public IPathSegment NextSegment { get; set; }
            ...
            protected int CalcWinding(RegionComposer composer)
            {
                ...
                foreach (var originalPath in composer.FindOriginalPathsAtRect(new AARect2DD(nextSegmentMidpoint, nextSegmentMidpoint)))
                {
        ...
        {
            public VertexNode(Point2DD point)
            {
                Point = point;
            }

            public Point2DD Point { get; }
        ...
        {
            public SegmentNode(IPathSegment originalSegment)
            {
                OriginalSegment = originalSegment;
            }

            public IPathSegment OriginalSegment { get; }
        ...
        {
            public SegmentSplit(double distance, IPathSegment segment)
            {
                Distance = distance;
                Segment = segment;
            }
        ...
        {
            public AreaInfo(AARect2DD rect)
            {
                Rect = rect;
            }

            public AARect2DD Rect { get; }
            public HashSet<IPathSegment> OriginalSegments { get; } = new HashSet<IPathSegment>();
            public HashSet<IPath> OriginalPaths { get; } = new HashSet<IPath>();
        }
    }
}
