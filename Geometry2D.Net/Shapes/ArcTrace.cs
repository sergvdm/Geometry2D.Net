...
namespace Svd.Geometry2D.Shapes
{
    internal class ArcTrace : TraceSegment, IArcTrace
    {
        ...
        protected override void BuildRegion(IGeometry2DEngine geometry2DEngine, IRegionBuilder regionBuilder, IPathBuilder pathBuilder, bool invert)
        {
            ...
            var hw = Width / 2;
            var pt1 = Guide.Center + new Vector2DD(Guide.StartAngle) * (Guide.Radius + hw);
            var pt12 = Guide.Center + new Vector2DD(Guide.StartAngle + Guide.SweepAngle / 2) * (Guide.Radius + hw);
            var pt2 = Guide.Center + new Vector2DD(Guide.EndAngle) * (Guide.Radius + hw);
            var pt23 = Guide.EndPoint + Guide.Tangent(Guide.EndPoint).Unit() * hw;
            var pt3 = Guide.Center + new Vector2DD(Guide.EndAngle) * (Guide.Radius - hw);
            var pt34 = Guide.Center + new Vector2DD(Guide.EndAngle - Guide.SweepAngle / 2) * (Guide.Radius - hw);
            var pt4 = Guide.Center + new Vector2DD(Guide.StartAngle) * (Guide.Radius - hw);
