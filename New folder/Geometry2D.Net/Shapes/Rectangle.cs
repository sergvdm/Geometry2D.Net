namespace Altium.Geometry2D.Shapes
{
    ...
    {
        public Rectangle(Point2DD vertex0, Point2DD vertex2, double rotation)
            : this(new RectInitData(vertex0, vertex2, rotation))
        {
        }

        public Rectangle(Point2DD center, double width, double height, double rotation)
    ...
    {
        public RectInitData(Point2DD vertex0, Point2DD vertex2, double rotation)
        {
            Rotation = rotation.NormalizeRadAngle();
            Center = new Line(vertex0, vertex2).Midpoint();
            var centerLine = new InfiniteLine(Center, new Vector2DD(rotation));
            var vertex1 = vertex2.MirrorBy(centerLine);
            var vertex3 = vertex0.MirrorBy(centerLine);
            Vertices = new Point2DD[4];
            Vertices[0] = vertex0;
            Vertices[1] = vertex1;
            Vertices[2] = vertex2;
            Vertices[3] = vertex3;
        ...
        public RectInitData(Point2DD center, double width, double height, double rotation)
        {
            Center = center;
            Width = width;
            Height = height;
            Rotation = rotation.NormalizeRadAngle();
            var v1 = Width * 0.5 * new Vector2DD(Rotation);
            var v2 = Height * 0.5 * new Vector2DD(Rotation + Math2D.HPI);
            Vertices = new Point2DD[4];
