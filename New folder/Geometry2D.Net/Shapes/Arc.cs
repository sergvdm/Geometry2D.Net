...
{
    internal class Arc : PathSegment, IArc
    {
        public Arc(Point2DD startPoint, Point2DD endPoint, Point2DD pointOnArc)
        {
            ...
            {
                Center = new Point2DD((startPoint.X + pointOnArc.X) / 2, (startPoint.Y + pointOnArc.Y) / 2);
            ...
            else
            {
                ...
                Center = new Point2DD(cx + pointOnArc.X, cy + pointOnArc.Y);
            ...
            if (!new Vector2DD(Center, pointOnArc).Angle.BelongsToAngularSpan(StartAngle, SweepAngle))
                SweepAngle += Math2D.DPI * (-Math.Sign(SweepAngle));
            if ((StartPoint == EndPoint || SweepAngle == 0) && Radius > 0) SweepAngle = Math2D.DPI;
        }

        public Arc(Point2DD startPoint, Point2DD endPoint, double radius, bool cw = false, bool longest = false)
        {
            if (radius < 0)
            ...
            else
            {
                ...
                if (h2 < 0) h2 = 0;
                var h = Math.Sqrt(h2);
                var m = new Point2DD((startPoint.X + endPoint.X) / 2, (startPoint.Y + endPoint.Y) / 2);
        ...
        public Arc(Point2DD center, double radius, double startAngle, double sweepAngle)
        {
            if (radius < 0)
                throw new ArgumentException("Invalid radius", nameof(radius));
            ...
            StartPoint = Center + new Vector2DD(StartAngle) * Radius;
            EndPoint = Center + new Vector2DD(EndAngle) * Radius;
        }

        internal Arc(Point2DD startPoint, Point2DD endPoint, double radius, Point2DD center, bool cw = false)
        {
            StartPoint = startPoint;
            EndPoint = endPoint;
            Center = center;
        ...
        public Point2DD Center { get; private set; }
        ...
        {
            return Center + Radius * new Vector2DD(StartAngle + SweepAngle / 2);
        }
        ...
        public override double LengthFromStart(Point2DD pt)
        {
            var pointSweepAngle = StartAngle.MinAngularSweepTo(new Vector2DD(Center, pt).Angle);
        ...
        public override Vector2DD Tangent(Point2DD pt)
        {
            return SweepAngle >= 0 ? new Vector2DD(Center, pt).Ortogonal() : new Vector2DD(pt, Center).Ortogonal();
        }

        public override double Curvature(Point2DD pt)
        {
            var c = 1 / Radius;
        ...
        {
            var splitAngle = new Vector2DD(Center, pt).Angle;
            ...
            var pointOnArc1 = Center + new Vector2DD(StartAngle + arc1SweepAngle / 2) * Radius;
            var pointOnArc2 = Center + new Vector2DD(splitAngle + arc2SweepAngle / 2) * Radius;
            var arc1 = new Arc(StartPoint, pt, Radius, Center, SweepAngle < 0);
            var arc2 = new Arc(pt, EndPoint, Radius, Center, SweepAngle < 0);
            return new PathVertex(arc1, arc2);
        }
        ...
        public override int WindingNumber(IGeometry2DEngine geometry2DEngine, Point2DD pt)
        {
            ...
            var ir = Math2D.Intersection(new Point2DD(Center.X - 1, pt.Y), new Point2DD(Center.X + 1, pt.Y), Center, Radius);
